...
function generateGreeting(userInfo: any): string {
  ...
  if (!history || history.visit_count <= 1) {
    if (name) {
      return `안녕하세요, ${name}님! 차의과학 대학교, 경영학 전공, 에이아이 가이드입니다. ${name}님의 방문을 환영합니다! 전공에 대해 궁금한 게 있으시면, 편하게 물어보세요!`;
    }
    return `안녕하세요! 차의과학 대학교, 경영학 전공, 에이아이 가이드입니다. 전공에 대해 궁금한 게 있으시면, 편하게 물어보세요!`;
  }
